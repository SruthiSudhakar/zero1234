...
from mesh_metrics import evaluate_meshes, evaluate_meshes_safe
...
from voxnerf.data import load_rtmv_data
...
class SJC(BaseConf):
    ...
    nerf_path:        str = "/home/rliu/Desktop/cvfiler04/datasets/RTMV/google_scanned"
...
def sjc_3d(poser, vox, model: ScoreAdapter,
    ...
    input_image, input_pose, input_mask, input_mesh_path = load_rtmv_data(nerf_path, scene, index)
    assert input_mesh_path is not None
    try:
        os.symlink(os.path.join(nerf_path, scene), os.path.join(folder_name, 'source_data'))
    except Exception as e:
        pass
    model.input_mesh_path = input_mesh_path
...
def evaluate(score_model, vox, poser):
    ...
    # evaluate CD and IoU
    res = evaluate_meshes_safe(score_model.input_mesh_path, save_mesh_path)
